-- 修正 get_blocked_slots：以台灣時區 (Asia/Taipei) 解讀 p_date，避免「選 2/5 台灣」卻查 UTC 2/5 導致日期錯位
-- 在 Supabase Dashboard → SQL Editor 執行（若已執行過 migration-get-blocked-slots.sql，執行本檔即可覆寫）
...
COMMENT ON FUNCTION get_blocked_slots(UUID, DATE) IS '回傳該分店指定「台灣日」內已被預訂的時段（Asia/Taipei 解讀 p_date），供 API 計算可預約時段';
